...
public class MeasureHelpSha1ServerResource extends VersusServerResource {
    ...
    public static final String PATH_TEMPLATE = "/measures/" + '{' + ID_PARAMETER + '}' + "/helpsha1";

    @Get
    public String retrieve() {
        String id = (String) getRequest().getAttributes().get(ID_PARAMETER);
        try {
            return ((ServerApplication) getApplication()).getMeasureHelpSha1(id);
        } catch (NotFoundException ex) {
            Logger.getLogger(MeasureHelpSha1ServerResource.class.getName()).log(Level.SEVERE, null, ex);
            setStatus(Status.CLIENT_ERROR_NOT_FOUND);
            return "Measure help " + id + " not found";
        }
    }
}
