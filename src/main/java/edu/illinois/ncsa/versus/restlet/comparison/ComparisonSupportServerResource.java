...
public class ComparisonSupportServerResource extends ServerResource {
    ...
    public boolean retrieve() {
        ...
        String adapterId = (String) attributes.get(ADAPTER_PARAMETER);
        String extractorId = (String) attributes.get(EXTRACTOR_PARAMETER);
        String measureId = (String) attributes.get(MEASURE_PARAMETER);
        ...
        for(Slave slave : server.getSlaves()) {
            if(slave.supportComparison(adapterId, extractorId, measureId)) {
                return true;
            }
        }
        return false;
