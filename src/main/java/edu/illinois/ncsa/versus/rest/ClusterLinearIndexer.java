...
public class ClusterLinearIndexer implements Serializable,Indexer{
	private static Log log = LogFactory.getLog(ClusterLinearIndexer.class);
	...
	List<Descriptor> indescriptors;
	List<String> inidentifiers;
	
	public ClusterLinearIndexer() {
		cluster = new ClusterTreeExpanded();
		descriptors = new ArrayList<Descriptor>();
		identifiers = new ArrayList<String>();
		
		indescriptors=new ArrayList<Descriptor>();
		inidentifiers=new ArrayList<String>();
		...
		try {
			...
			if (location != null) {
				...
				log.debug("Cluster Linear indexer folder=" + indexerfolder);
			} else {
	...
	public void addDescriptor(Descriptor document, String id) {
		...
		indescriptors.add(document);
		inidentifiers.add(id);
		log.debug("indes sizes="+indescriptors.size());
	}
    ...
    if(cluster.identifiers.length!=0){
		...
		if(!inidentifiers.isEmpty()){
			for(int i=0;i<inidentifiers.size();i++){
			   try {
				   		Result.setProximity(getMeasure().compare(indescriptors.get(i), query));
				   		Result.setDocId(inidentifiers.get(i));
				   		log.debug(inidentifiers.get(i));
				   		searchResults.add(Result);
				   		Result = new SearchResult();
			   		} catch (Exception e) {
	...
	public List<SearchResult> query(Descriptor query, Measure measure) {
		...
		if(cluster!=null){
				...
				log.debug("Cluster is not Empty: After Query: Results.size=" + results.size());
		...
		if(!inidentifiers.isEmpty()){
			log.debug("Not Empty: inidentifiers size: "+inidentifiers.size());
			for(int i=0;i<inidentifiers.size();i++){
			   try {
				   		Result.setProximity(getMeasure().compare(indescriptors.get(i), query));
				   		Result.setDocId(inidentifiers.get(i));
